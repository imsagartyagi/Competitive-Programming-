...
public class MaximumSumInSubArray{
    ...
    private static node query(node[] tree, int start, int end, int left, int right, int treenode) {
        ...
        ans.maxSum=Math.max(leftnode.maxSum,Math.max(rightnode.maxSum,Math.max(leftnode.sum+rightnode.prefixSum,Math.max(rightnode.sum+leftnode.suffixSum,
                Math.max(leftnode.suffixSum+rightnode.prefixSum,leftnode.sum+rightnode.sum) ))));
        ans.prefixSum= Math.max(leftnode.sum,Math.max(leftnode.prefixSum,leftnode.sum+rightnode.prefixSum));;
        ans.sum=leftnode.sum+rightnode.sum;;
        ans.suffixSum=Math.max(rightnode.sum,Math.max(rightnode.suffixSum,rightnode.sum+leftnode.suffixSum));;
        return ans;
    }
    ...
    private static void fillTree(node[] tree, node leftnode,node rightnode,int treenode) {
        ...
        tree[treenode].prefixSum= Math.max(leftnode.sum,Math.max(leftnode.prefixSum,leftnode.sum+rightnode.prefixSum));
        tree[treenode].suffixSum= Math.max(rightnode.sum,Math.max(rightnode.suffixSum,rightnode.sum+leftnode.suffixSum));
